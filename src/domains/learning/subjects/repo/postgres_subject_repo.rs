...
impl SubjectRepository for PostgresSubjectRepo {
    ...
    async fn db_get_all_subjects(&self) -> sqlx::Result<Vec<Subject>, Error> {
        let subjects = sqlx::query_as("SELECT * FROM subjects ORDER BY title")
    ...
    async fn db_delete_subjects(&self, ids: Vec<String>) -> sqlx::Result<u64, Error> {
        let result = sqlx::query("DELETE FROM subjects WHERE id = ANY($1)")
            .bind(&ids)
