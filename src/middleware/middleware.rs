...
use crate::middleware::claims::{JWTClaims, JwkSet};



pub async fn verify_token(token: &str) -> Result<JWTClaims, jsonwebtoken::errors::Error> {
    let url = std::env::var("AWS_COGNITO_JWK_JSON").expect("AWS_COGNITO_JWK_JSON not set or can't be read correctly"); // Update this URL

    let response = reqwest::Client::new()
        .get(url)
        .send()
        .await.expect("Failed requesting JWK JSON")
    ...
    let user_pool = std::env::var("AWS_COGNITO_USER_POOL_ID").expect("AWS_COGNITO_USER_POOL_ID must be set");
    let client_id = std::env::var("AWS_COGNITO_CLIENT_ID").expect("AWS_COGNITO_USER_POOL_ID must be set");
...
pub async fn middleware(req: HttpRequest) -> actix_web::Result<JWTClaims, Error> {
    match req.headers().get(actix_web::http::header::AUTHORIZATION).and_then(|t| { t.to_str().ok() }) {
        Some(auth_header) => {
            match auth_header.strip_prefix("Bearer ") {
                Some(token) => {
                    match verify_token(token).await {
                        ...
                        Err(e) => match e.kind() {
                            ExpiredSignature => {
                                Err(Error::other("The token is expired"))
                            }
